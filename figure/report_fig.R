...
load("../R/estimation_report.rda")

ltab$param <- 1:nrow(ltab)

estimation_report <- dplyr::select(estimation_report, -betamean, -alpha)

full <- merge(ltab, estimation_report, by="param")
...
ggsave("report_fig.pdf", gcomb, width=10, height=6)
